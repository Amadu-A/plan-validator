# services/retrieval-service/src/retrieval_service/transport/middleware.py

"""Pure ASGI request correlation middleware Retrieval Service."""

import logging
import re
import time

from plan_validator_common.observability import new_correlation_id, scoped_log_context
...
class RequestContextMiddleware:
    """Изолирует correlation/request context одного HTTP request."""
    ...
    async def __call__(
        ...
        """Создаёт bounded log context только для HTTP protocol."""
        if scope["type"] != "http":
        ...
        async def send_with_context(message: Message) -> None:
            """Добавляет request identifiers в HTTP response headers."""
