...
from embedding_service.infrastructure.gpu_lease import (
    CrossProcessFileGpuLease,
)
from embedding_service.infrastructure.gpu_runtime import (
...
from embedding_service.infrastructure.model_cache import (
    HuggingFaceModelCacheProbe,
)
...
class EmbeddingWorkerActivity:
    """Отслеживает active GPU deliveries для graceful drain."""

    def __init__(
        self,
    ) -> None:
        """Создаёт idle worker state."""
        self._active_jobs = 0
    ...
    def job_finished(
        ...
        """Освобождает active delivery slot."""
    ...
    *,
    ...
    async def handle(
        ...
        try:
            ...
            resolved_activity.job_finished()
...
async def run_worker(
    ...
    """Consume'ит GPU queue с bounded graceful shutdown."""
    single_use_case, batch_use_case = build_use_cases(settings)

    activity = EmbeddingWorkerActivity()
    ...
    try:
        ...
        def request_shutdown() -> None:
            """Перестаёт принимать новые GPU jobs и запускает drain."""
            shutdown_event.set()
        ...
        try:
            await asyncio.wait_for(
                activity.wait_idle(),
                timeout=(settings.embedding_queue.graceful_shutdown_seconds),
            )

        except TimeoutError:
            _LOGGER.warning(
                "Embedding worker graceful drain timed out",
                extra={
                    "event": ("embedding_worker_drain_timeout"),
                    "timeout_seconds": (settings.embedding_queue.graceful_shutdown_seconds),
                },
            )

        else:
            _LOGGER.info(
                "Embedding worker graceful drain completed",
                extra={
                    "event": ("embedding_worker_drain_completed"),
                },
            )
